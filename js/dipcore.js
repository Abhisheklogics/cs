class ColorQuantizer {
  ...
  info() {
    return `${this.numColors} colors | ${this.levelsPerCh} levels/ch | step=${this.step.toFixed(1)}`;
  }
}
...
class MedianCutQuantizer {
  ...
  quantize(imageData) {
    ...
    const palette = this._medianCut(pixels, this.numColors);
    return this._applyPalette(imageData, palette);
  }
    ...
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        ...
        if (this.quantizer instanceof MedianCutQuantizer) {
          const tmp = new Uint8ClampedArray([r, g, b, 255]);
          const small = new ImageData(tmp, 1, 1);
          const res = this.quantizer.quantize(small);
          qr = res.data[0]; qg = res.data[1]; qb = res.data[2];
        } else {
...
class DiffImage {
  static compute(orig, quant, gain) {
    ...
    for (let i = 0; i < out.length; i += 4) {
      ...
      out[i] = Math.min(255, amp); out[i+1] = Math.round(amp * 0.35); out[i+2] = 0; out[i+3] = 255;
    }
...
class ImageMetrics {
  ...
  static ssim(a, b) {
    ...
    for (let i = 0; i < a.data.length; i += 4) {
      ...
      sA+=la; sB+=lb; sA2+=la*la; sB2+=lb*lb; sAB+=la*lb;
    }
    const muA=sA/n, muB=sB/n;
    const vA=sA2/n-muA*muA, vB=sB2/n-muB*muB, cov=sAB/n-muA*muB;
  ...
  static forward(block) {
    ...
    for (let u = 0; u < 8; u++) {
      for (let v = 0; v < 8; v++) {
        ...
        for (let x = 0; x < 8; x++)
          for (let y = 0; y < 8; y++)
            s += block[x*8+y] * Math.cos((2*x+1)*u*Math.PI/16) * Math.cos((2*y+1)*v*Math.PI/16);
  ...
  static inverse(coeffs) {
    ...
    for (let x = 0; x < 8; x++) {
      for (let y = 0; y < 8; y++) {
        ...
        for (let u = 0; u < 8; u++)
          for (let v = 0; v < 8; v++)
            s += DCT8x8.C(u)*DCT8x8.C(v)*coeffs[u*8+v] * Math.cos((2*x+1)*u*Math.PI/16) * Math.cos((2*y+1)*v*Math.PI/16);
        out[x*8+y] = 0.25 * s;
      }
    }
    return out;
...
class Filters {
  static applyKernel(imageData, kernel, kSize) {
    ...
    const out=new Uint8ClampedArray(src.length), half=Math.floor(kSize/2);
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        ...
        for (let ky = 0; ky < kSize; ky++) {
          for (let kx = 0; kx < kSize; kx++) {
            ...
            const pi=(py*W+px)*4, kv=kernel[ky*kSize+kx];
            sumR+=src[pi]*kv; sumG+=src[pi+1]*kv; sumB+=src[pi+2]*kv;
          }
        }
        const i=(y*W+x)*4;
  ...
  static sharpen(imageData) {
    return Filters.applyKernel(imageData, [0,-1,0,-1,5,-1,0,-1,0], 3);
  }

  static emboss(imageData) {
    return Filters.applyKernel(imageData, [-2,-1,0,-1,1,1,0,1,2], 3);
  }
  ...
  static sobelEdge(imageData) {
    ...
    for (let y=1;y<H-1;y++) for (let x=1;x<W-1;x++) {
      ...
      for (let r=0;r<3;r++) for (let c=0;c<3;c++) {
        const pi=((y+r-1)*W+(x+c-1))*4;
        const l=0.299*src[pi]+0.587*src[pi+1]+0.114*src[pi+2];
        gx+=l*kx[r*3+c]; gy+=l*ky[r*3+c];
      }
  ...
  static medianFilter(imageData) {
    ...
    for (let y=1;y<H-1;y++) for (let x=1;x<W-1;x++) {
      ...
      for (let dy=-1;dy<=1;dy++) for (let dx=-1;dx<=1;dx++) {
        ...
        rs.push(src[pi]); gs.push(src[pi+1]); bs.push(src[pi+2]);
      }
      rs.sort((a,b)=>a-b); gs.sort((a,b)=>a-b); bs.sort((a,b)=>a-b);
      const i=(y*W+x)*4;
      out[i]=rs[4]; out[i+1]=gs[4]; out[i+2]=bs[4]; out[i+3]=src[i+3];
    }
    return new ImageData(out, W, H);
  }

  static brightnessContrast(imageData, brightness, contrast) {
    brightness=brightness||0; contrast=contrast||1;
    const src=imageData.data, out=new Uint8ClampedArray(src.length);
    for (let i=0;i<src.length;i+=4) {
  ...
  static histogramEqualize(imageData) {
    const src=imageData.data, out=new Uint8ClampedArray(src.length);
    const n=src.length/4, hist=new Array(256).fill(0);
    for (let i=0;i<src.length;i+=4) {
    ...
    for (let i=0;i<src.length;i+=4) {
      const l=Math.round(0.299*src[i]+0.587*src[i+1]+0.114*src[i+2]);
      const newL=lut[l], ratio=l>0?newL/l:1;
