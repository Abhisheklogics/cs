...
class DiffImage {
  static compute(orig, quant, gain = 2) {
    ...
    for (let i = 0; i < out.length; i += 4) {
      ...
      out[i]   = Math.min(255, amp);           // R (orange)
      out[i+1] = Math.round(amp * 0.35);       // G
      out[i+2] = 0;                            // B
      out[i+3] = 255;
    }
...
class DCT8x8 {
  ...
  static lumaQuantMatrix(quality = 50) {
    const base = [
    ...
    return base.map(v => Math.max(1, Math.min(255,
      Math.floor((v * scale + 50) / 100))));
  }
}
...
class Filters {
  ...
  static gaussianBlur(imageData, sigma=1) {
  ...
  static threshold(imageData,t=128) {
    const src=imageData.data,out=new Uint8ClampedArray(src.length);
    for (let i=0;i<src.length;i+=4) {
      const l=Math.round(0.299*src[i]+0.587*src[i+1]+0.114*src[i+2]);
      const v=l>=t?255:0;
  ...
  static brightnessContrast(imageData,brightness=0,contrast=1) {
    const src=imageData.data,out=new Uint8ClampedArray(src.length);
    for (let i=0;i<src.length;i+=4) {
